...
//const geojsonWithCentroid = require('germany_with_centroids.json');
const fs = require('fs');
...
// Lade die JSON-Datei asynchron und parse sie
const filePath = path.resolve(__dirname, 'germany_with_centroids.json');
fs.readFile(filePath, 'utf8', (err, data) => {
  ...
  try {
    const geojsonWithCentroid = JSON.parse(data);
...
function printCSV(data, filename) {
  ...
  fs.writeFile(filename + '.csv', resultCSV, (err) => {
    if (err) {
      console.error(err);
    }
  });
...
function printGeoJson(data, geoJson, filename) {
  ...
  fs.writeFile(filename + '.geojson', JSON.stringify(geoJson), (err) => {  
    if (err) {
      console.error(err);
    }
  });
