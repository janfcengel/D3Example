...
d3.json('gosper_curve_data_4.json').then(Y => {
    ...
    const nodes = Y.map((d, i) => ({ id: i, x: xScale(d[0]), y: yScale(d[1]) }));
    const links = nodes.map((node, i) => i < nodes.length - 1 ? { source: i, target: i + 1 } : null).filter(d => d !== null);
    ...
    const node = svg.append("g")
        ...
        .attr("class", "node")
        .attr("r", 3)
        .style("fill", d => d.id >= 100 && d.id <= 200 ? "blue" : "red");
